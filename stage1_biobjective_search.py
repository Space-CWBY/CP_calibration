...
"""
Standalone Stage-1 bi-objective search driver (NSGA-II style).
...
# ===== BEGIN ENTRYPOINT: run_stage1_biobjective_search.py =====
...
def main() -> None:
    ...
    ap.add_argument("--scratch-dir", type=Path, default=Path("./scratch_stage1"))
    ...
    ap.add_argument("--export-curves", action="store_true")
    args = ap.parse_args()

    runtime = RuntimePaths(args.evpsc_root, args.evpsc_exe, args.exp_dir, args.template_dir, args.scratch_dir)
    options = Stage1Options(timeout_sec=None if args.timeout <= 0 else int(args.timeout), parallel_cases=int(args.parallel_cases), w_rmse=float(args.w_rmse), w_dshape=float(args.w_dshape), export_curves=bool(args.export_curves))
    evaluator = Stage1Evaluator(runtime, options, args.out_dir)
    run_stage1_biobjective_search(
        evaluator,
    ...
    print(f"Done. Trials written to: {evaluator.trials_csv}")


if __name__ == "__main__":
    main()

# ===== END ENTRYPOINT: run_stage1_biobjective_search.py =====
